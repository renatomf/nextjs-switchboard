...
| ~~O token público do Trigger.dev que a página do workflow gera vale 1 hora e nunca é renovado: um canvas aberto por mais tempo para de receber atualizações das runs. No teste com duas abas, as duas ficaram com o Stop parado até o F5 (o Sentry não registrou a queda da assinatura, então a causa não foi confirmada)~~ | `workflow-runs-provider.tsx` | ✅ | Resolvido: a assinatura pede um token novo ao servidor (`refreshAccessToken`) quando o Trigger.dev recusa o antigo, e um vigia refaz a assinatura quando o servidor diz que a run mostrada como ativa já terminou |
