...
| **Integração de IA** | `anthropic/claude-opus-4-8` por padrão, trocável por `STAGEHAND_MODEL` (Claude, Gemini ou um Ollama local), roteado pelo Stagehand — `act` · `extract` · `observe` · `agent` |
...
flowchart TB
    ...
    subgraph BB["🌐 Browserbase · Stagehand v3"]
        ...
        Modelo["modelo de STAGEHAND_MODEL<br/>Claude por padrão"]
...
- **O modelo depende de uma chave própria do provedor.** O `STAGEHAND_MODEL` escolhe o modelo
  (`anthropic/…` por padrão, `google/…` ou `ollama/…`), e a chave vem da variável do provedor:
  `CLAUDE_API_KEY` ou `GEMINI_API_KEY`. Sem ela, a run falha logo no início dizendo qual variável
